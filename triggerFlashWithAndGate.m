function triggerFlashWithAndGate()
FPS = 20;
...
frameClkChannel.DutyCycle = .50; %.20 -> 10ms
% pfiTerminal = frameClkChannel.Terminal;
% trigTerminalString = [devName, '/', pfiTerminal];
...
obj.pulseTime = flashOnTime;


startCamFcn = @() frameClkSession.startBackground;
queueFlashFcn = @() obj.sendPulse;
flashQueuedFcn = @() fprintf('*********FLASH******* (Queued...)\n');
stopCamFcn = @() frameClkSession.stop;

stopCamTimer = timer(...
   'ExecutionMode','singleShot',...
   'StartDelay', interFlashTime,...
   'TimerFcn', @(~,~) cellfun(@feval, {stopCamFcn}));

flashTimerFcn = timer(...
   'ExecutionMode','fixedRate',...
   'Period', interFlashTime,...
   'StartDelay', preFlashTime,...
   'TasksToExecute',flashNumRepeats,...
   'StartFcn', @(~,~) cellfun(@feval, {startCamFcn}),...
   'TimerFcn', @(~,~) cellfun(@feval, {queueFlashFcn, flashQueuedFcn}),...
   'StopFcn', @(~,~) start(stopCamTimer));

assignin('base','flashTimerFcn',flashTimerFcn)
