classdef TonePuffSystem2 < SubSystem
   ...
   methods % SETUP
	  function obj = TonePuffSystem2(varargin)
		 if nargin > 1
	  ...
	  function createSystemComponents(obj)
		 ...
		 if isempty(obj.interTrialInterval)
			if numel(obj.interTrialIntervalRange) == 2
			   ...
			   warning('TonePuffSystem2:NoInterTrialIntervalRange', 'No automatic generation of Inter-Trial Intervals without set range')
			end
		 end
   ...
   methods % CONTROL
	  function start(obj)
		 ...
		 if isempty(obj.frameSyncListener)
			warning('TonePuffSystem2:start:NoFrameSyncListener',...
		 ...
		 fprintf('TonePuffSystem2 STARTED\n');
	  end
	  function stop(obj)
		 % (Required by SubSystem Parent Class)
   ...
   methods % TONE-PUFF SPECIFIC
	  function loadStandardStimulus(obj)
		 ...
		 randomStimNumbers = nonRandomStimNumbers(randi(numel(nonRandomStimNumbers), [obj.nTrials,1]));
		 obj.trialStimulusNumber = randomStimNumbers;
		 % 		 randomVector = rand([obj.nTrials, 1]);
		 % 		 obj.trialStimulusNumber(randomVector < .5) = 1;
		 % 		 obj.trialStimulusNumber(randomVector >= .5) = 2;
   ...
   methods % EVENT RESPONSE
	  function experimentStateChangeFcn(obj,~,evnt)
		 % (Required by SubSystem Parent Class)
		 fprintf('TonePuffSystem2: Received ExperimentStateChange event\n')
	  ...
	  function trialStateChangeFcn(obj,~,~)
		 % (Required by SubSystem Parent Class)
		 fprintf('TonePuffSystem2: Received TrialStateChange event\n')
		 ...
		 try
			...
			elseif frameNum >= obj.trialFirstFrame(end) %last frame
			   ...
			   if (hat - timeAtStart) < (.9/obj.frameClkFrequency)
				  if (obj.puffObj.sessionObj.ScansQueued < 1) && (obj.toneObj.sessionObj.ScansQueued < 1)
					 obj.puffObj.prepareOutput(obj.puffSignal{obj.nextStimulusNumber});
				  end
			   end
			   if (hat - timeAtStart) < (.9/obj.frameClkFrequency)
