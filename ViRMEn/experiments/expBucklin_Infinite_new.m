function code = expBucklin_Infinite_new
...
% Randomize Laser On Times
fr = 20;
laser.duration = 3*fr; %secs
laser.spacingMin = 20*fr;%20*fr; %secs
laser.spacingRange = 20*fr;
laser.numEvents = 80;
laser.frameOn = round(cumsum(laser.spacingMin + randi(laser.spacingRange,laser.numEvents,1)));
laser.frameOff = laser.frameOn + 100;
laser.currFrame = 0;
t = clock;
save(sprintf('%s/laser_time_indices_%d-%d-%d_%d_%d_%d.mat',vr.vrSystem.currentDataSetPath,...
    t(1),t(2),t(3),t(4),t(5),t(6)),'laser');
vr.laser = laser;

%laser.timeOn = cumsum(laser.spacingMean + laser.spacingStdDev*randn(laser.numEvents,1));

%framesOn = randi([0 1], 20*60*35, 1);
...
% increment
vr.laser.currFrame = vr.laser.currFrame + 1;
if ismember(vr.laser.currFrame,vr.laser.frameOn)
    disp('FIRE!');
    disp(['Frame is : ' num2str(vr.laser.currFrame)]);
%     vr.vrSystem.rewardPulseObj.sendPulse();
    vr.vrSystem.rewardPulseObj.sessionObj.outputSingleScan(...
        vr.vrSystem.rewardPulseObj.pulseVal); % (set high)

end
if ismember(vr.laser.currFrame,vr.laser.frameOff)
    disp('OFF');
    vr.vrSystem.rewardPulseObj.resetPulseAnalog();
end
end
